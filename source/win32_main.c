...
internal char *
Win32LoadEntireFileAndNullTerminate(char *filename)
{
    char *result = 0;
    ...
    HANDLE file = {0};
    {
        ...
        if((file = CreateFile(filename, desired_access, share_mode, &security_attributes, creation_disposition, flags_and_attributes, template_file)) != INVALID_HANDLE_VALUE)
        {
            ...
            if(read_bytes)
            {
                result = Win32HeapAlloc(read_bytes+1);
                DWORD bytes_read = 0;
...
internal void *
Win32LoadEntireFile(char *filename, int *size)
{
    void *result = 0;
    ...
    HANDLE file = {0};
    {
        ...
        if((file = CreateFile(filename, desired_access, share_mode, &security_attributes, creation_disposition, flags_and_attributes, template_file)) != INVALID_HANDLE_VALUE)
        {
            ...
            if(read_bytes)
            {
                result = Win32HeapAlloc(read_bytes);
                DWORD bytes_read = 0;
                OVERLAPPED overlapped = {0};
                ReadFile(file, result, read_bytes, &bytes_read, &overlapped);
                *size = (int)bytes_read;
